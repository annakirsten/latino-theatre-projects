...
function get_production_dates() {
    ...
    rewind_posts(); // stop loop one
    query_posts('post_type=event'); // show productions 
    
    while (have_posts()) : the_post(); // Begin Current Productions
        ...
        $production_date = $start_month_word . ' ' . $start_day . ' ' . $start_year . ' to ' . $end_month_word . ' ' . $end_day . ' ' . $end_year;
...
function current_production( $today_date, $compare_date, $production_date ){
    rewind_posts(); // stop loop one
    query_posts('post_type=event'); // show productions 
    ...
    while (have_posts()) : the_post(); // Begin Current Productions
        ...
        if($today_date<$compare_date) :
            ...
            $productionLink = get_permalink($post->ID);
            $productionTitle = $post->$post_title;
            $productionThumb = $post->the_post_thumbnail('thumbnail');

            echo '<ul>';

                echo '<h5 class="production_title"><a href="' . $productionLink . '" rel="bookmark" title="Permanent Link to ' . $productionTitle . '">' . $productionTitle . '</a></h5>';

                echo '<h5 class="production_dates">' . $production_date . ' </h5>';

                echo '<a href="' . $productionLink . '" rel="bookmark" title="Permanent Link to ' . $productionTitle . '">' . $productionThumb . '</a>';

            echo '</ul>';

        endif; // end compare if
